...
class ImageUrlList:
    ...
    def dump_to_file(self, city, harvest_start_time_for_filename):
        ...
        file = open("harvests/image-temp/" + filebase + ".txt", "w")
    ...
    if threads_were_added_to_this_warc:
        ...
        if image_url_list.we_got_images:
            ...
            call(["wget", "-q", "--level=0", "--warc-cdx", "--page-requisites",
                "--directory-prefix=harvests/image-temp/wget-warc-temp/",
                "--warc-file=harvests/image-temp/" + filebase,
                "--warc-max-size=1G",
                "-i", "harvests/image-temp/" + filebase + ".txt"])

            # Cleanup garbage-files generated by wget
            call(["bash", '-c',
                'rm ./harvests/image-temp/*-meta.warc.gz 2>/dev/null'])
            call(["bash", '-c',
                'rm ./harvests/image-temp/*.cdx 2>/dev/null'])
            call(["bash", '-c',

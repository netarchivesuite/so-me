...
calculate_top_x() {
    ...
    case "$TOPTYPE" in
        tags)
            zcat -f "$@" | jq -r '.entities.hashtags[].text' | tr '[[:upper:]]' '[[:lower:]]' | sort | uniq -c | sort -rn > "$T"
            ;;
        links)
            zcat -f "$@" | jq -r '.entities.urls[].expanded_url' | sort | uniq -c | sort -rn > "$T"
            ;;
        mentions)
            zcat -f "$@" | jq -r '.entities.user_mentions[].screen_name' | sort | uniq -c | sort -rn > "$T"
            ;;
        authors_by_tweets)
